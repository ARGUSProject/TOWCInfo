...
pswfc() {
  ...
  echo -en "Enter IP:"
  read psip
  echo "Scanned by" $USER "On" ${date} "With the LAN IP" ${LANE} > ~/Desktop/${psip}_SCANinfo.txt
  echo "*********************************************************" >> ~/Desktop/${psip}_SCANinfo.txt
  nmap -A ${psip} >> ~/Desktop/${psip}_SCANinfo.txt
  echo "End of Scan" >> ~/Desktop/${psip}_SCANinfo.txt
  sleep 3
  menu
}
hitlan() {
...
portrange() {
    ...
    read p2
    echo "Scanning Port" ${p1} "through" ${p2} "On IP" ${PRIP} > ~/Desktop/${p1}_${p2}_${PRIP}_PORTinfo.txt
    echo "Scanned by" $USER "On" ${date} "With the LAN IP" ${LANE} >> ~/Desktop/${p1}_${p2}_${PRIP}_PORTinfo.txt
...
allPorts() {
    ...
    #echo -en "Through:"
    #read p2
    echo "Scanned by" $USER "On" ${date} "With the LAN IP" ${LANE} >> ~/Desktop/${LANIP}_PORTinfo.txt
    echo "Scanning"${LANIP}"!" >> ~/Desktop/${LANIP}_PORTinfo.txt
    echo "*********************************************************" >> ~/Desktop/${LANIP}_PORTinfo.txt
...
scanTool() {
    ...
    echo "Scanned by" $USER "On" ${date} "With the LAN IP" ${LANE} > ~/Desktop/${NSL}_info.txt
    echo "Scanning"${NSL}"!" >> ~/Desktop/${NSL}_info.txt
